...
int choix_epreuve(int nbepreuve,char tabepreuve[maxepreuve][255]);
// 5 eme procedure
...
{
    int trouv = 0 ;
    int i = indice_epreuve(epreuve,tabepreuve,nbepreuve);
    if(i>=nbepreuve){
        return trouv;
    }else{
        trouv = 1;
...
int choix_epreuve(int nbepreuve,char tabepreuve[maxepreuve][255]){
    ...
    do{
        clrscr();
        int i =0;
        while((i < 30) && (i+(s*30) < nbepreuve)){
            printf("%d: %s",(i+1 +(s*30)),tabepreuve[i+(s*30)]);
            i++;
        }
        printf("=============================================================================\n\n");
        gotoxy(5,33);
        ...
        do{
            ...
            case 34:
                if(z == 13){
                    ...
                    do{
                    a = 1;
                    printf("Entrer le nom d'epreuve  -->   ");
                    fgets(choix,255,stdin);
                    printf("%s",choix);
                    if(!verif_epreuve(choix,tabepreuve,nbepreuve)){
                     printf("Cette epreuve n'exite pas dans les jeux olympiques\n\n");
                }
                }while(!verif_epreuve(choix,tabepreuve,nbepreuve));
                }else if((z == 0x50) || (z == 0x48)){
                    clrscr();
                    int i =0;
                    while((i < 30) && (i+(s*30) < nbepreuve)){
        ...
        //==================
    return indice_epreuve(choix,tabepreuve,nbepreuve);
}
...
void listjo(ptr tabjo[maxepreuve][maxpays],char tabpays[maxpays][255],int nbpays,char tabepreuve[maxepreuve][255],int nbepreuve){
     clrscr();
     int nume = choix_epreuve(nbepreuve,tabepreuve);
     clrscr();
     if(!sansath(nume,tabjo,nbpays)){
        printf("%s",tabepreuve[nume]);
        for(int j=0;j<nbpays;j++){
            textcolor(GREEN);
            printf("%s",tabpays[j]);
            textcolor(WHITE);
            if(longeur(tabjo[nume][j]) == 0){
                printf("Ce pay n'a aucun athlethe en l'epreuve %s\n",tabepreuve[j]);
            }else{
                afficher(tabjo[nume][j]);
            }
            printf("\n---------------------------------------------------------------------------------------------------------------------------------------------\n");
        }
     }
